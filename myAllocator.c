...
/*
  ...
  Such a block can be marked as allocated by setting its "allocated"
  ...
  FindFirstAllocRegion() uses findFirstFit to locate a sufficiently
  large unallocated block.  This block will be split if it contains
...
BlockPrefix_t *findbestFit(size_t s) {	/* find best block with best usable space > s */
	BlockPrefix_t *p = arenaBegin;
	BlockPrefix_t *fit;
	...
	while (p) {
		if(!p->allocated){
			...
			if(computeUsableSpace(p) > s){
				...
				if((computeUsableSpace(p) - s ) < (computeUsableSpace(fit)-s) ){
					fit = p;
				}
...
void *resizeRegion(void *r, size_t newSize) {
	...
	void *nextRegion = getNextPrefix(r);
	int nextRegionSize = computeUsableSpace(nextRegion);

	if (r != (void *)0)		/* old region existed */
		oldSize = computeUsableSpace(regionToPrefix(r));
	else oldSize = 0;		/* non-existant regions have size 0 */


	//New loop
	/*
if (!nextRegion->allocated && (nextRegionSize + oldsize) >= newSize ){
	BlockPrefix_t *oldPrefix;
	size_t growSize = newSize-oldsize;

	r = makeFreeBlock(oldPrefix + needed , newsize);
  }
	 */
	//
